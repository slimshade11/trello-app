...
import { take, Observable, tap, combineLatest, map, takeUntil, skipWhile, filter } from 'rxjs';
...
export class BoardComponent extends DestroyComponent implements OnInit {
    ...
    loadData(): void {
        ...
        this.boardsFacade.loadBoardById$(this.boardId).pipe(takeUntil(this.destroy$)).subscribe();

        this.boardsFacade.loadColumns$(this.boardId).pipe(takeUntil(this.destroy$)).subscribe();

        this.boardsFacade.loadTasks$(this.boardId).pipe(takeUntil(this.destroy$)).subscribe();
    }
    ...
    initializeListeners(): void {
        ...
        this.boardsFacade.listenToUpdateBoardName$().pipe(takeUntil(this.destroy$)).subscribe();

        this.boardsFacade.listenToDeleteBoardById$().pipe(takeUntil(this.destroy$)).subscribe();

        this.boardsFacade.listenToDeleteColumnById$().pipe(takeUntil(this.destroy$)).subscribe();

        this.boardsFacade.listenToUpdateColumn$().pipe(takeUntil(this.destroy$)).subscribe();
    ...
    openTask(taskId: string, columnId: string): void {
        ...
        this.boardData$
            .pipe(
                take(1),
                tap(({ tasks }: BoardData) => {
                    taskName = tasks.find(task => task.id === taskId)?.title.toUpperCase();
                })
            )
            .subscribe();
